...
- **Iteración 2** (`feature/iter-2-leyenda-deciles-global`): 4 cambios al reporte PDF. Plan en `~/.claude/plans/lee-los-docs-claude-md-refactored-kazoo.md`.
  - **T1 — Untrack `__pycache__`.** `git rm --cached -r` de los 31 archivos `*.pyc` que llevaban rastreados desde antes de que `.gitignore:2` los excluyera. Limpia `git status` que se ensuciaba en cada `pytest`/`import`.
  - **T2 — Leyenda de severidad en el PDF.** Nuevo `_build_severity_legend()` en `report/builder.py` que devuelve 3 entradas (CRITICAL/WARNING/OK) con bullets de las reglas, leyendo los counts actuales desde `config/settings.py` (no hardcoded). Campo nuevo `AnalysisContext.severity_legend`. Tabla `.severity-legend-table` en `fleet_report.html` debajo de "Estado por Segmento" con badges idénticos. 5 tests nuevos en `test_status_aggregation.py`: orden, labels ES, presencia de los counts en el texto.
  - **T3 — Violaciones de orden sobre deciles (no bins fijos).** Refactor de orquestación: el cómputo+persistencia de deciles se mueve del builder al `MetricsCalculator._calculate_segment_metrics` (un solo cálculo por segmento-semana). `check_decile_ordering_violations(decile_table, ascending, tol=0.005)` y `load_per_target_deciles(...)` en `decile_metrics.py`. Calculator detecta inversiones del `event_rate` decil-a-decil reutilizando el dict en memoria. Builder solo lee `FACT_DECILES_HISTORY` para AMBAS gráficas (consolidada y per-target). Como la consolidada multi-target ya no funcionaba (solo dibuja un target hoy), se simplifica a "consolidada del target primario" sin recompute al vuelo. Eliminadas `check_ordering_violations` y `get_ordering_violations_for_metric` de `business_metrics.py` (único callsite ya migrado). `FactMetricsHistory.details` ahora trae `decile_low/high` + `cohort_window_start/end`; `calculated_from="FACT_DECILES_HISTORY"`. Tests: nuevo `test_decile_ordering.py` con 12 casos (incl. roundtrip persist→load); eliminadas las clases `TestCheckOrderingViolations` y `TestOrderingViolations` de `test_business_metrics.py`. **Fixture inflado** en `conftest._insert_performance_individual`: 25 créditos por bin × 10 bins = 250 por segmento (vs 10 antes), suficiente para `DECILE_MIN_OBS=100`.
  - **T4 — Gini/KS global por target.** Nueva `get_gini_ks_global(session, model_id, origination_week, target_name, score_max_by_registry)` en `performance.py`: combina TODOS los segmentos del modelo en una origination_week (= calculation_week - lag), invierte score por crédito según el `score_max` de SU segmento (robusto a heterogeneidad futura), rankea conjunto, reutiliza `compute_gini_ks_individual`. Sin persistencia en BD — cálculo al vuelo en el builder. Campo nuevo `AnalysisContext.global_performance`. Nueva tabla "Métricas Globales por Target" arriba de "Estado por Segmento" en `fleet_report.html`, reusando `.fleet-status-table`. 8 tests en `test_performance_global.py`.
  - **Verificación E2E:** rebuild SQLite local (`bootstrap_v2` 11 segs · 139 vars · 808 baseline). ETL ventana rodante 4 semanas (`2026-03-16..2026-04-06`). `run_pipeline.py --date 2026-04-06 --no-email --no-llm` → `10 OK | 1 WARNING | 0 CRITICAL`. PDF generado en `artifacts/reports/mlmonitor_2026-04-06.pdf`. Smoke del builder confirma global_performance con valores realistas: b_malo4_6 gini=0.31 (n=15,737), b_malo8_13 gini=0.36 (n=17,522), b_malo14_26 gini=0.23 (n=28,328). FACT_METRICS_HISTORY confirma `calculated_from="FACT_DECILES_HISTORY"` y `details.violation_pairs[].decile_low/high` poblados.
  - **Suite:** 146/146 passed (128 previos + 5 leyenda + 12 decile_ordering + 8 perf_global - 7 obsoletos de ordering_violations en business_metrics).
  - **Pendiente:** (a) drop+rebuild en RDS productivo (requiere autorización explícita); (b) re-correr `scripts/backfill.py` post-rebuild; (c) deuda anotada: crear `FACT_GLOBAL_METRICS` (o hacer `model_registry_id` nullable) si se quiere histórico semanal del Gini/KS global; (d) revisar por separado por qué `render_consolidated_decile_chart` actualmente solo dibuja un target (probable bug en la lógica de merge por `credito_id`, fuera del scope de esta iteración).

---

## 2026-05-07

- **Iteración 1 paso A1+A2 de `upgrades.md`** (`feature/upgrades`): preparación para multi-modelo. Plan en `~/.claude/plans/rosy-tumbling-biscuit.md`.
  - **A1 — eliminar `MODEL_ID="BAZBOOST_V1"` hardcodeado.** Se borraron 5 defaults: `pipeline/orchestrator.py:25`, `data/incremental_etl.py:47`, `data/bootstrap.py:39`, y los argparse de `run_incremental_etl.py` y `run_pipeline.py`. Nuevo módulo `src/mlmonitor/data/model_registry.py` con helper `resolve_model_ids(session, explicit)`: si `explicit` viene poblado retorna `[explicit]`; si no, retorna **todos** los `model_id` distintos con `valid_to IS NULL` en `META_MODEL_REGISTRY`. Si no hay activos, levanta `ValueError` con mensaje claro. Los scripts `run_*.py` ahora llaman al helper y loopean sobre la lista resultante (`for model_id in model_ids: ...`).
  - **A2 — `primary_target` a `META_MODEL_REGISTRY`.** Nueva columna `primary_target_variable: String(100), nullable=True` (sin FK a `META_VARIABLES` por SCD2). Se borró la constante `PRIMARY_TARGET = "b_malo14_26"` de `bootstrap.py:60` y el import en `report/builder.py`. El builder lee `model_regs[0].primary_target_variable` con fallback a "target con lag mediano". Se corrigió el **bug latente**: `AnalysisContext.primary_target` ya no tiene default `"b_malo8_13"` — ahora es requerido (movido en el orden del dataclass para respetar la regla de Python "no-default antes de default"). Único callsite es `report/builder.py:304` con kwargs, no rompe nada.
  - **Bootstrap explícito por modelo.** `ModelBootstrap.__init__` recibe `model_id` y `primary_target` como kwargs requeridos; `ModelBootstrapV2` los forwardea via `super().__init__`. `run_bootstrap.py` y `run_bootstrap_v2.py` agregan los args `--model-id` y `--primary-target` `required=True`. Sin defaults: el operador declara qué modelo está bootstrapeando.
  - **`docker/entrypoint.sh`:** sin cambio funcional. Soporte opcional para `MODEL_ID` env var (si está definida, se inyecta `--model-id $MODEL_ID`; si no, los scripts iteran sobre todos los modelos activos — comportamiento default deseado para multi-modelo).
  - **Tests:** nuevo `tests/test_model_registry.py` con 7 casos: vacío + explicit=None → ValueError, un modelo → singleton, dos modelos → ambos sorted, explicit override, explicit pasa-through (no valida DB), filtra `valid_to IS NULL`, dedup por DISTINCT (varios segmentos → un solo model_id en resultado). `tests/conftest.py::_insert_registry` agrega `primary_target_variable=TARGET_NAME` al fixture de `MetaModelRegistry`.
  - **Suite de tests:** `poetry run pytest` → **104 passed, 6 failed**. Los 6 fallos son **pre-existentes** en `test_status_aggregation.py` (verificado con `git stash + pytest`) — son tests del cambio del 2026-05-05 a los umbrales (`status_*_count_*` 1/3/4 → 8/5/8) que no se actualizaron. **Fuera del scope de A1+A2**, deuda técnica a resolver aparte. Los 7 tests nuevos del helper pasan.
  - **Verificación local end-to-end:**
    1. `rm mlmonitor_dev.db && poetry run python scripts/run_bootstrap_v2.py --db-url sqlite:///mlmonitor_dev.db --model-id BAZBOOST_V1 --primary-target b_malo14_26` → 11 segmentos · 139 vars · 216 thresholds · 808 baseline rows. Las 11 filas de `META_MODEL_REGISTRY` tienen `primary_target_variable='b_malo14_26'` poblado.
    2. `run_incremental_etl.py --date 2026-04-06 --db-url sqlite:///mlmonitor_dev.db` (sin `--model-id`) → log `[etl] Modelos a procesar: ['BAZBOOST_V1']`, 808 distribution + 126 binned + 61587 individual rows.
    3. `run_pipeline.py --date 2026-04-06 --db-url sqlite:///mlmonitor_dev.db --no-email --no-llm` → 322 métricas, PDF generado en `artifacts/reports/mlmonitor_2026-04-06.pdf`, flota `9 OK | 1 WARNING | 1 CRITICAL`. `AnalysisContext.primary_target == 'b_malo14_26'` (resuelto desde DB, no de import). PDF subió a S3 (comportamiento esperado: `S3_BUCKET` está en Secrets Manager).
    4. Caso de error: DB vacía + `run_pipeline.py` sin `--model-id` → `ValueError: No hay modelos activos en META_MODEL_REGISTRY. ¿Olvidaste correr el bootstrap? Ejecuta: poetry run python scripts/run_bootstrap_v2.py --model-id <ID> --primary-target <target>`.
  - **Pendiente:** (a) aplicar drop+rebuild a RDS productivo (requiere autorización explícita, **no aplicado**); (b) re-correr `scripts/backfill.py` post-rebuild para repoblar histórico semanal en RDS; (c) ADR formal en `docs/decisions.md §8.2.30` documentando los cambios; (d) actualizar `CLAUDE.md §5` con los nuevos comandos de bootstrap; (e) deuda técnica separada: arreglar los 6 tests de `test_status_aggregation.py` que fallan por umbrales viejos.

- **Iteración 1 paso A5+A6+fix tests** (`feature/upgrades`, mismo día, plan extendido en `~/.claude/plans/rosy-tumbling-biscuit.md`). A5 y A6 unificados: A6 supersede al A5 mencionado en `upgrades.md` (no hay flag `--variables-config <path>`; el path de la config se auto-resuelve por convención desde `--model-id` ya requerido tras A1).
  - **Nuevo directorio por modelo:** `data/inputs/model_configs/bazboost_v1/` con 4 archivos: `config.json` (todas las constantes pequeñas del modelo: nombre, tipo, owner, score_bins, score_min/max, missing_sentinel, num_bins_numeric, primary_target, lista de targets con lag y ascending_order, lista de segmentos con sus variables canónicas y feature_count, name_mapping SERC→canónico, extra_serc_variables, categorical_variables); `variable_descriptions.csv` (movido desde `raw_tables/Dicionario_Variables_BB.csv`); `segment_descriptions.csv` (movido desde `Dicionario_Segmentos_BB.csv`); `thresholds.csv` (movido desde `tresholds_monitoreo.csv`, typo del nombre corregido). Los 4 archivos van versionados en git (excepción explícita en `.gitignore`) — son código del modelo, no datos crudos. La separación arquitectónica queda clara: config = git, datos crudos semanales = S3 sync.
  - **Nuevo módulo `src/mlmonitor/data/model_config.py`:** dataclasses `ModelConfig`, `TargetConfig`, `SegmentConfig`. Loaders: `from_json_file(path)` con validación (campos requeridos, primary_target ∈ targets, categorical_variables ⊆ segments.variables, score_bins consistentes), `for_model(model_id, base_dir=None)` que auto-resuelve `<base_dir>/<model_id_lowercase>/config.json`. Métodos helper: `score_bin_labels`, `score_bin_cuts`, `segment_ids`, `segment_id_int`, `segment_by_id`, `is_categorical`, `target_names`, `target_by_name`, `serc_to_canonical` (cache lazy del mapa reverso). El método `serc_to_canonical` reemplaza la función global del módulo `variable_mapping` eliminado.
  - **`src/mlmonitor/data/variable_mapping.py` ELIMINADO.** Toda su funcionalidad (`CANONICAL_VARIABLES`, `SEGMENT_GROUP_NAMES`, `SEGMENT_FEATURE_COUNTS`, `_SPECIAL_CASES`, `EXTRA_SERC_VARIABLES`, `serc_to_canonical`, `get_canonical_variables_for_segment`) pasó al JSON o a métodos de `ModelConfig`. Cero importadores activos antes de borrarlo.
  - **`bootstrap.py` refactorizado:** se eliminaron las constantes módulo `MODEL_NAME`, `MODEL_TYPE`, `OWNER_TEAM`, `SCORE_BINS`, `SCORE_BIN_LABELS`, `SCORE_BIN_CUTS`, `MISSING_SENTINEL`, `NUM_BINS_NUMERIC`, `TARGET_VARIABLES`. El constructor recibe `config: ModelConfig` (en lugar de `model_id` y `primary_target` separados que A1+A2 introdujeron — la nueva interfaz es más limpia). Todos los `range(1, 12)` reemplazados por `for seg in self.config.segments`. La detección de variables categóricas (`vname == "fisexo"` hardcodeado) pasa a `self.config.is_categorical(vname)` que lee del JSON. Las descripciones se cargan desde `self.config.variable_descriptions_csv` y `self.config.segment_descriptions_csv` (paths derivados del `config_dir`, no del `raw_dir`).
  - **`bootstrap_v2.py` refactorizado:** mismo patrón, hereda de `ModelBootstrap` y forwardea `config` al `super().__init__`. Imports muertos eliminados.
  - **`incremental_etl.py` refactorizado:** recibe `config: ModelConfig`. La función global `serc_to_canonical` (importada del módulo eliminado) se reemplaza por `self.config.serc_to_canonical`. El literal `MISSING_SENTINEL = -100` pasa a `self.config.missing_sentinel`.
  - **`threshold_loader.py` refactorizado:** las funciones `parse_thresholds_csv`, `expected_metrics_for_segment`, `compute_thresholds_for_segment`, `_normalize_metric_name` ahora reciben `config: ModelConfig` en lugar de importar `TARGET_VARIABLES`, `CANONICAL_VARIABLES`, `EXTRA_SERC_VARIABLES`, `serc_to_canonical` directamente. Los defaults (`DEFAULT_PSI`, `DEFAULT_NULL_RATE`, etc.) se mantienen como constantes módulo (no son específicos del modelo). El `expected_metrics_for_segment` ahora recibe `segment_id` (str) en lugar de int — usa `config.segment_by_id`.
  - **Scripts CLI:** `run_bootstrap.py` y `run_bootstrap_v2.py` ya no requieren `--primary-target` (viene del JSON); cargan `ModelConfig.for_model(args.model_id)`. `run_incremental_etl.py` carga el config dentro del loop sobre `model_ids`. `run_pipeline.py` no requiere cambios (no instancia bootstrap/ETL; consume todo de la DB).
  - **Tests adaptados:** `tests/test_threshold_loader.py` ahora usa fixture `ModelConfig.for_model("BAZBOOST_V1")` en lugar de imports directos de `TARGET_VARIABLES` y `CANONICAL_VARIABLES`. Las funciones reciben `config` como argumento explícito. 10/10 pasan.
  - **Tests nuevos:** `tests/test_model_config.py` con 20 casos: carga válida, validaciones (campo faltante, primary_target inexistente, categorical no presente en segmentos, score_bin con orden inválido, targets vacío, archivo inexistente), `for_model` auto-resolve y mensaje claro de error, score_bin_labels/cuts derivados, segment_ids/segment_id_int (incl. error con string mal formado), segment_by_id (incl. error si no existe), is_categorical, target_helpers, y 5 casos de `serc_to_canonical` (INTERCEPTO → None, name_mapping, fuzzy match, unknown → None, smoke contra config real BAZBOOST_V1).
  - **Fix de los 6 tests rotos en `tests/test_status_aggregation.py`** (deuda pre-existente del cambio de umbrales 2026-05-05): reescritos para parametrizar con `settings.status_*_count_*` en lugar de hardcodear 1/3/4. Ahora son robustos a cambios futuros: `test_criticas_iguales_al_warning_threshold_es_warning` construye `settings.status_crit_count_to_warning` alertas críticas y verifica WARNING; `test_criticas_iguales_al_critical_threshold_es_critical` análogo; tests adicionales de "bajo threshold" para cubrir fronteras. 15/15 pasan.
  - **Suite total:** `poetry run pytest` → **131 passed, 0 failed** (vs 104+6 antes). Suma: 75 previos + 6 reescritos + 7 de model_registry + 20 de model_config + 10 de threshold_loader adaptados + 13 más de la suite previa. Cero regresiones.
  - **Verificación end-to-end (local):**
    1. `rm mlmonitor_dev.db && poetry run python scripts/run_bootstrap_v2.py --db-url sqlite:///mlmonitor_dev.db --model-id BAZBOOST_V1` → 11 segs · 139 vars · 216 thresholds · 808 baseline rows. Sin pasar `--primary-target` (viene del JSON).
    2. ETL en ventana rodante de 4 semanas: `2026-03-16`, `2026-03-23`, `2026-03-30`, `2026-04-06` (cada uno con `--db-url sqlite:///mlmonitor_dev.db`).
    3. Pipeline final con `--date 2026-04-06 --no-email --no-llm`.
    4. Verificación visual del PDF.
  - **Pendiente:** drop+rebuild en RDS productivo (autorización explícita del usuario), re-bootstrap RDS, backfill semanas históricas en RDS, smoke test ECS.
