...
def test_direction_canonical():
    ...
    assert _direction_for("gini_b_malo2_4") == "lower_worse"
    assert _direction_for("ks_b_malo8_13") == "lower_worse"
    assert _direction_for("ordering_violations_b_malo14_52") == "higher_worse"
...
def test_normalize_metric_name():
    ...
    assert _normalize_metric_name("gini_b_malo2_4") == "gini_b_malo2_4"
    # SERC → canónico
...
def test_parse_thresholds_csv_filters_and_maps(tmp_path: Path):
    rows = [
        ...
        "3,gini_b_malo2_4,bb_1,0.3,0.2,lower_worse,01/01/25,",
    ...
    assert lookup[("s1", "gini_b_malo2_4")] == (0.3, 0.2)
...
def test_expected_metrics_for_segment_counts():
    n_targets = len(TARGET_VARIABLES)  # 6
...
def test_compute_thresholds_uses_defaults_when_missing():
    ...
    assert (by_name["gini_b_malo2_4"]["warning_threshold"], by_name["gini_b_malo2_4"]["critical_threshold"]) == DEFAULT_GINI_TARGET
    assert (by_name["ks_b_malo2_4"]["warning_threshold"], by_name["ks_b_malo2_4"]["critical_threshold"]) == DEFAULT_KS_TARGET
    assert (by_name["ordering_violations_b_malo2_4"]["warning_threshold"], by_name["ordering_violations_b_malo2_4"]["critical_threshold"]) == DEFAULT_ORD_TARGET
...
def test_compute_thresholds_overrides_direction_in_csv():
    ...
    csv_lookup = {("s1", "gini_b_malo2_4"): (0.3, 0.2)}
    rows = compute_thresholds_for_segment("s1", registry_id=1, csv_lookup=csv_lookup)
    target_row = next(r for r in rows if r["metric_name"] == "gini_b_malo2_4")
...
def test_total_count_across_all_segments():
    ...
    expected_total = (
        11 * (2 + len(TARGET_VARIABLES) * 3)  # 11 × (psi + null_rate + 6×3)
...
def test_parse_thresholds_csv_real_file():
    ...
    # 11 segmentos × (2 básicas + 6×3 targets) = 220 filas mínimas que SÍ deben mapear.
    # Más las gini_<scorecard_var> que mapean. Total esperado >> 220.
    assert len(lookup) > 220
